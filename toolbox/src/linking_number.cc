...
namespace LK {
  ...
  Angle<T>::build(
    ...
    if ( (y1 >= 0 && y2 >= 0) || (y1 <= 0 && y2 <= 0) ) {
      ...
      LK_ASSERT(
        abs(y1) > u_epsi_y || x1 > u_epsi_x,
        "Angle::build, ambiguos angle\nx1 = " << x1 << "\ny1 = " << y1 <<
        "\n(probable intersection)"
      );
      LK_ASSERT(
        abs(y2) > u_epsi_y || x2 > u_epsi_x,
        "Angle::build, ambiguos angle\nx2 = " << x2 << "\ny2 = " << y2 <<
        "\n(probable intersection)"
      );
  ...
  template <typename T>
  void
  LinkingNumber<T>::close_curve( unsigned ncurve, int_type weight ) {
    LK_ASSERT(
      ncurve < curves.size(),
  ...
  LinkingNumber<T>::evaluate(
    ...
    LK_ASSERT(
      i_curve < curves.size() && j_curve < curves.size(),
  ...
  template <typename T>
  void
  LinkingNumber<T>::evals(
    unsigned const i_curve[],
    unsigned       ni,
    unsigned const j_curve[],
    unsigned       nj,
    int_type       mat[]
  ) {
